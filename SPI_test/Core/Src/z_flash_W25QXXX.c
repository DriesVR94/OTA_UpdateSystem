 ...
 * 			dataSize	number of bytes to write
 ***********************************************************************/
void SPI_Flash_Write(uint32_t addr, uint8_t* data, uint32_t dataSize){
uint8_t buffer[4];
uint16_t quota;
...
/***********************************************************************
 * @BRIEF   Function to copy code from external memory (Flash) to internal memory
 *          It reads code from Flash, splits it into pages, and writes to the specified area in internal memory
 * @PARAM   addr_flash   Starting address in external memory (Flash) to read from
 *          addr_internal   Starting address in internal memory to write to
 *          code_size   Number of bytes of code to copy
 ***********************************************************************/
...
void CopyCodeFromSPIFlash(uint32_t addr_ext_flash, uint32_t addr_internal_flash, uint32_t code_size) {
    ...
    FirstSector = GetSector(addr_internal_flash);
    NbOfSectors = GetSector(final_addr_internal) - FirstSector + 1;
    ...
    printf("Internal memory erase performed successfully \r\n");
    /* Program the user Flash area word by word*/
    // Temporary buffer for writing to internal Flash memory
    uint32_t write_buffer[INTERNAL_FLASH_PAGE_SIZE / sizeof(uint32_t)];

    while (remaining_size > 0) {
        // Calculate the size to read in this iteration (up to the page size)
        uint32_t read_size = (remaining_size > EXT_FLASH_PAGE_SIZE) ? EXT_FLASH_PAGE_SIZE : remaining_size;

        // Read data from external Flash
        SPI_Flash_Read(addr_ext_flash, buffer, read_size);

        // Convert the read data to a 32-bit uint buffer
        for (uint32_t i = 0; i < read_size / sizeof(uint32_t); i++) {
            write_buffer[i] = *((uint32_t*)(buffer + (i * sizeof(uint32_t))));
        }

        // Write the data to internal Flash
        for (uint32_t i = 0; i < read_size / sizeof(uint32_t); i++) {
            HAL_StatusTypeDef status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, addr_internal, write_buffer[i]);
            if (status != HAL_OK) {
                printf("Error in Flash Program \r\n");
                return;
            }
            addr_internal += sizeof(uint32_t);
        }

        // Move to the next page in external Flash
        addr_ext_flash += read_size;
        remaining_size -= read_size;
    }
    printf("Flash memory write performed \r\n");
    HAL_FLASH_Lock();
}
...
/*
void CopyCodeFromExternalFlashToInternal(uint32_t addr_ext_flash, uint32_t addr_internal_flash, uint32_t code_size) {
    uint8_t buffer[EXT_FLASH_PAGE_SIZE]; // Buffer for reading from external Flash
    uint32_t remaining_size = code_size;
    uint32_t addr_internal = addr_internal_flash;
    ...
    while (remaining_size > 0) {
        ...
        // Read data from external Flash
        Flash_Read(addr_ext_flash, buffer, read_size);

        // Write the read data to internal Flash
        uint32_t write_size = 0;
        while (write_size < read_size) {
            // Calculate the size to write in this iteration (up to the page size)
            uint32_t size_to_write = (read_size - write_size > INTERNAL_FLASH_PAGE_SIZE) ? INTERNAL_FLASH_PAGE_SIZE : read_size - write_size;

            // Write data to internal Flash
            HAL_StatusTypeDef status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, addr_internal, (uint64_t)&buffer[write_size], size_to_write);
            if (status != HAL_OK) {
                // Handle error, such as retry mechanism or logging
                return;
            }

            // Move to the next page
            addr_internal += size_to_write;
